...
double findmax_doubleparser(std::vector<std::string> &s) {
    double answer = 0;
    double x = 0;
    for (std::string &st : s) {
        auto p = fast_double_parser::parse_number(st.data(), &x);
        if (p == nullptr) {
            throw std::runtime_error("bug in findmax_fastfloat");
        }
        answer = answer > x ? answer : x;
...
void process(std::vector<std::string> &lines, size_t volume) {
    ...
    pretty_print(volume, lines.size(), "fast_double_parser", time_it_ns(lines, findmax_doubleparser, repeat));
#ifdef FROM_CHARS_AVAILABLE_MAYBE
...
void parse_random_numbers(size_t howmany, bool concise) {
    auto a = -1;
    auto b = 1;
    std::cout << "# parsing random integers in the range [" << a << "," << b << ")" << std::endl;
    std::vector<std::string> lines;
